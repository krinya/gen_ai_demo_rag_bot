...
if __name__ == "__main__":
    ...
    from packages.chatbot.prompts.router_prompts import STRUCTURED_ROUTER_PROMPT, ANSWER_GRADING_PROMPT, QUESTION_REPHRASE_PROMPT
else:
    # Local imports for module usage
...
class TemplateChatbot:
    ...
    async def chat_with_metadata(self, user_input: str, context: str = "") -> Dict[str, Any]:
        """
        Enhanced chat method that returns both response and routing metadata
        ...
        Returns:
            Dictionary containing response and routing metadata
        """
        try:
            # Create initial state - LangGraph will handle message persistence automatically
            initial_state = ChatbotState(
            ...
            routing_metadata = {
                "primary_route": result.get("route_decision", "unknown"),
                "final_source": result.get("current_source", "unknown"),
                "answer_quality": result.get("answer_quality", "unknown"),
                "rephrase_attempts": result.get("rephrase_attempts", 0),
                "failed_sources": result.get("failed_sources", []),
                "original_question": result.get("original_question", user_input),
                "final_question": result.get("question", user_input)
            }
            ...
            logger.error(f"Error in chat_with_metadata method: {e}")
            return {
